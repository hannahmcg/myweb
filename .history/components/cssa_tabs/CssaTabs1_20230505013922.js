import {
  ...
  export default function CssaTabs1() {
    ...
    return (
      <div>
        <Tabs value="html" className="max-w-[40rem] mx-auto text-blue-900">
          <TabsHeader 
          ...
          indicatorProps={{
            ...
            {data.map(({ label, value }) => (
              <Tab key={value} value={value} defaultValue={data[0].value} className="hover:text-blue-400 text-sm text-clip md:text-base lg:text-base">         
                {label}            
              </Tab>
            ))}
          </TabsHeader>
          <TabsBody animate={{
            ...
            {data.map(({ value, questions, selected }) => (
